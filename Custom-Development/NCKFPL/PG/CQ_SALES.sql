...
SELECT row_number() over() as                  uk,
       ...
       D.ENTRY_TYPE L6_ENTRY_TYPE,
	   D.DOCUMENT_TYPE L6_DOCUMENT_TYPE,
	   D.PINVCODE L6_PINVCODE,
	   D.INVOICE_DATE L6_INVOICE_DATE,
	   D.INVOICE_NO L6_INVOICE_NO,
	   D.PCODE L6_PCODE,
	   D.AMOUNT L6_AMOUNT,
	   D.ADJUSTED L6_ADJUSTED,
	   D.PENDING L6_PENDING,
	   D.overdue_days L6_overdue_days
  FROM (                                                       /*Header Part*/
        SELECT A.INVCODE                            INVCODE,
          ...
          FROM SALINVMAIN A
               ...
               INNER JOIN ADMSITE OS1 ON (A.ADMSITE_CODE_OWNER = OS1.CODE)
        ...
        where ( a.invcode = NULLIF('@DocumentId@','')::int)) H
       RIGHT JOIN
       (                                                            /*Detail*/
        SELECT 'L2#DETAIL'           LVL,
               ...
               GINVIEW.fnc_get_batch_string_new (
                     'select b.BATCH_SERIAL_NO, a.INVQTY qty, b.BATCH_SERIAL_MODE,b.code
           FROM SALINVDET a 
          ...
          WHERE  a.invcode = '
                  ...
                  || CHR (39)
                  || ' AND A.rate = '
                  || COALESCE (invoice_rate, 0)::text
                  || ' AND A.mrp = '
                  || COALESCE (INVOICE_RSP, 0) ::text
                  || ' order by b.code')
                  batch_serial_string,
               DIVISION,
               SECTION,
               ...
               null ENTRY_TYPE,
			     null DOCUMENT_TYPE,
			     null PINVCODE,
			     null::DATE INVOICE_DATE,
			     null INVOICE_NO,
			     null::INT PCODE,
			     0 AMOUNT,
			     0 ADJUSTED,
			     0 PENDING,
			     null::TEXT overdue_days
          FROM (  SELECT INVCODE             INVCODE,
                    ...
                    FROM (SELECT *
                            FROM SALINVDET
                           where ( invcode = NULLIF('@DocumentId@','')::int)) M
                         ...
                         (  SELECT SALINVDET_CODE,
                              ...
                              FROM SALINVCHG_ITEM
                             WHERE     ISTAX = 'Y'
                                   AND ( invcode = NULLIF('@DocumentId@','')::int)
                          GROUP BY SALINVDET_CODE, APPAMT) TAX
                            ON (M.CODE = SALINVDET_CODE)
                   WHERE ( invcode = NULLIF('@DocumentId@','')::int)
                GROUP BY INVCODE,
                         M.CODE,
                         M.ICODE,
                         M.RATE,
        ...
        UNION ALL
          ...
          SELECT 'L4#HSN'           LVL,
                 ...
                 NULL            CHARGE_APPLICABLE_ON,
                 NULL            OPERATION_LEVEL,
                 SUM (INVQTY)::text    HSN_INVOICED_QUANTITY,
                 D.HSN_SAC_CODE  HSN_CODE,
                 H.HSN_SAC_CODE  HSN_SAC_ID,
                 DESCRIPTION     HSN_DESCRIPTION,
                 UNITNAME        HSN_UOM,
                 COALESCE (SUM (TAXABLE_AMOUNT), SUM (D.INVAMT))::text     HSN_TAXABLE_AMOUNT,
                 CGST_RATE::numeric      HSN_CGST_RATE,
                 SUM (CGST_AMOUNT)::numeric  HSN_CGST_AMOUNT,
                 SGST_RATE::numeric         HSN_SGST_RATE,
                 SUM (SGST_AMOUNT)::numeric  HSN_SGST_AMOUNT,
                 IGST_RATE::numeric         HSN_IGST_RATE,
                 SUM (IGST_AMOUNT)::numeric  HSN_IGST_AMOUNT,
                 CESS_RATE::numeric        HSN_CESS_RATE,
                 SUM (CESS_AMOUNT)::numeric HSN_CESS_AMOUNT,
                 NULL            DC_NO,
                 NULL::date            DC_DATE,
                 NULL            SO_NO,
                 NULL::date            SO_DATE,
            ...
            FROM (SELECT *
                    FROM SALINVDET
                   WHERE ( invcode = NULLIF('@DocumentId@','')::int)) D
        ...
        UNION ALL
          ...
          SELECT 'L5#REFDOCS'      LVL,
            ...
            FROM (SELECT *
                    FROM SALINVDET
                   WHERE ( invcode = NULLIF('@DocumentId@','')::int)) SI
           ...
           WHERE ( si.invcode = NULLIF('@DocumentId@','')::int)
        GROUP BY SI.INVCODE,
                 DCM.SCHEME_DOCNO,
                 DCM.DCDT,
                 ORM.SCHEME_DOCNO,
                 ...
                 ORM.DOCDT
                 UNION ALL
          /*PENDING INVOICE  PART*/
          SELECT 'L6#PENDING'      LVL,
                 5              SEQ,
                NULLIF('@DocumentId@','')::int           INVCODE,
                 NULL           DET_CODE,
                 NULL           ICODE,
                 NULL           DETAIL_INVOICE_QUANTITY,
                 NULL           DETAIL_HSN_SAC_CODE,
			     ...
			     PENDING,
			     overdue_days::TEXT
            FROM (/*|| Custom Development || Object : CQ_PENDING_INVOKICE || Ticket Id :  419937 || Developer : Dipankar ||*/
			SELECT
			       M.ENTRY_TYPE,
			       M.DOCUMENT_TYPE,
			       M.DOCUMENT_CODE PINVCODE,
