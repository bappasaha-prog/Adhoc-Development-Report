...
SELECT GINVIEW.FNC_UK () UK,
  ...
  FROM (  SELECT ICODE,
            ...
            FROM (  SELECT 1 DIP,
                           ...
                           F.BILL_MONTH
                      FROM (  SELECT D.ICODE,
                                     TO_CHAR (M.INVDT::DATE, 'MON-YY') BILL_MONTH,
                                     SUM (D.INVQTY)        QTY
                                FROM SALINVMAIN M
                                     INNER JOIN SALINVDET D
                                        ON M.INVCODE = D.INVCODE
                               WHERE     M.SALETYPE = 'O'
                                     AND M.INVDT::DATE BETWEEN  TO_DATE (
                                                                       '@ASON@',
                                                                       'YYYY-MM-DD') - INTERVAL '12 MONTH'
                                                             AND TO_DATE (
                                                                    '@ASON@',
                                                                    'YYYY-MM-DD')
                            GROUP BY D.ICODE, TO_CHAR (M.INVDT::DATE, 'MON-YY')
                            UNION ALL
                              SELECT D.ICODE,
                                     TO_CHAR (M.CSDATE::DATE, 'MON-YY') BILL_MONTH,
                                     SUM (D.QTY)            QTY
                                FROM SALCSMAIN M
                               ...
                               WHERE M.CSDATE::DATE BETWEEN TO_DATE (
                                                          ...
                                                          AND TO_DATE (
                                                                 '@ASON@',
                                                                 'YYYY-MM-DD')
                            GROUP BY D.ICODE, TO_CHAR (M.CSDATE::DATE, 'MON-YY')
                            UNION ALL
                              SELECT D.ICODE,
                                     TO_CHAR (M.SSDATE::DATE, 'MON-YY') BILL_MONTH,
                                     SUM (D.QTY)            QTY
                                FROM SALSSMAIN M
                               ...
                               WHERE M.SSDATE::DATE BETWEEN TO_DATE (
                                                          ...
                                                          AND TO_DATE (
                                                                 '@ASON@',
                                                                 'YYYY-MM-DD')
                            GROUP BY D.ICODE, TO_CHAR (M.SSDATE::DATE, 'MON-YY')) F
                 ...
                 (SELECT 1 DIP,
                    ...
                    FROM (  SELECT ROW_NUMBER () OVER (ORDER BY MCODE ASC) SEQ,
                             ...
                             WHERE DTFR BETWEEN (SELECT DTFR
                                                  ...
                                                  WHERE TO_DATE (
                                                                       '@ASON@',
                                                                       'YYYY-MM-DD') - INTERVAL '12 MONTH' BETWEEN DTFR
                                                                    AND DTTO)
                                            AND (SELECT DTFR
                                                   FROM ADMMONTH
                                                  WHERE TO_DATE ('@ASON@',
       ...
       (  SELECT K.ICODE,
                 ...
                 SUM (CASE WHEN K.ADMSITE_CODE = 1 THEN K.QTY ELSE 0 END)
                    BHIWANDI_STOCK_QTY,
                 SUM (CASE WHEN K.ADMSITE_CODE = 3730 THEN K.QTY ELSE 0 END)
                    MUMBAI_SKYCITY
            FROM INVSTOCK_ONHAND K
           WHERE     K.LOCCODE <> 2
