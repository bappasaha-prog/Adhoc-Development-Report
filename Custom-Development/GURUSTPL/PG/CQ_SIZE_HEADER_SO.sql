/*><><>< || Custom Development || Object : CQ_SIZE_HEADER_SO || Ticket Id : 390416 || Developer : Dipankar || ><><><*/
  SELECT GINVIEW.FNC_UK ()                                UK,
    ...
    FROM (SELECT DISTINCT
                 ...
                 DENSE_RANK ()
                    OVER (PARTITION BY D.ORDCODE ORDER BY TRIM (
                          CONCAT (LPAD (I.INVITEM_UDFSTRING07, 3, 0),
                                  I.CATEGORY3)))
                    SEQ
            FROM SALORDDET D INNER JOIN GINVIEW.LV_ITEM I ON D.ICODE = I.CODE
           where(ordcode in (
							select
