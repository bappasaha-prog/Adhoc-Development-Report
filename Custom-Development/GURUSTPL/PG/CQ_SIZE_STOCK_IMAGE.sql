...
/*><><>< || Custom Development || Object : CQ_SIZE_STOCK_IMAGE || Ticket Id : 394332 || Developer : Dipankar || ><><><*/
with SIZE as (
select
	ARTICLE_NAME,
...
union all
    ...
    SELECT unnest(regexp_matches('@#SizeStockImageItemNameMulti#@', '[^æ]+', 'g')) AS col1
    FROM generate_series(1, (SELECT count(*) FROM regexp_matches('@#SizeStockImageItemNameMulti#@', 'æ', 'g'))+1) AS g
)
or COALESCE (NULLIF ('@#SizeStockImageItemNameMulti#@', ''), '0')::text =0::text)
		union all
	select
            S.ARTICLE_NAME,
            coalesce(SD.repseq,
