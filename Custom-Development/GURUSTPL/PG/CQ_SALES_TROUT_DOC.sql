...
/*><><>< || Custom Development || Object : CQ_SALES_TROUT_DOC || Ticket Id : 394146 || Developer : Dipankar || ><><><*/
WITH site_data
...
SELECT ginview.fnc_uk()                  uk,
  ...
  FROM (                                                       /*Header Part*/
        SELECT A.INVCODE                            INVCODE,
         ...
         WHERE (a.invcode in (
		select
			(regexp_split_to_table('@DocumentId@',
			'[^|~|]+',
			'g'))::bigint as col1)
       ...
       (                                                            /*Detail*/
        SELECT 'L2#DETAIL'           LVL,
               ...
               NULL               SO_NO,
               NULL::date               SO_DATE
			   FROM (/* Formatted on 2025-02-21 11:56:05 (QP5 v5.294) */
				  SELECT D.INVCODE,
				         I.DEPARTMENT          STYLE,
				         I.CATEGORY1           BRAND,
