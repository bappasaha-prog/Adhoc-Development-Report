...
/*><><>< || Custom Development || Object : CQ_SALES_TROUT_DOC || Ticket Id : 394937 || Developer : Dipankar || ><><><*/
WITH site_data
...
SELECT ginview.fnc_uk()                  uk,
       ...
       D.DIS_RATE L2_DIS_RATE,
       D.ITEM_DISCOUNT_AMOUNT L2_ITEM_DISCOUNT_AMOUNT,
  ...
  FROM (                                                       /*Header Part*/
        SELECT A.INVCODE                            INVCODE,
          ...
          FROM SALINVMAIN A
          	   INNER JOIN (SELECT m.invcode,SUM (
                              COALESCE (m.invqty*m.rate, 0)) NETAMT2 from salinvdet m  WHERE (m.invcode  in (
    select
        ...
        ''),
        '0')::text = 0::text)
                            GROUP BY m.invcode) d ON a.invcode = d.invcode
                            LEFT JOIN (SELECT INVCODE ,SUM(CHGAMT) HCHGAMT FROM SALINVCHG WHERE (invcode  in (
    select
        unnest(regexp_matches('@DocumentId@',
        '[^|~|]+',
        'g'))::bigint as col1)
        or coalesce (nullif ('@DocumentId@',
        ''),
        '0')::text = 0::text) GROUP BY INVCODE ) HC ON A.INVCODE = HC.INVCODE
       ...
       (                                                            /*Detail*/
        SELECT 'L2#DETAIL'           LVL,
          ...
          FROM (  SELECT INVCODE             INVCODE,
                         ...
                         SUM (
                              (COALESCE (INVAMT, 0) + COALESCE (M.CHGAMT, 0))
                            + COALESCE (TAXAMT, 0))
                            ITEM_NET_AMOUNT,
                         ...
                         DIS.RATE DIS_RATE,
                         SUM(DIS.CHGAMT) ITEM_DISCOUNT_AMOUNT
                     FROM (SELECT *
                            FROM SALINVDET
                           WHERE (invcode  in (
    select
    ...
    select
        ...
        '0')::text = 0::text)
                          GROUP BY SALINVDET_CODE, APPAMT) TAX
                            ...
                            LEFT OUTER JOIN(  SELECT SALINVDET_CODE, CHGAMT,rate
								  FROM SALINVCHG_ITEM SI INNER JOIN SALCHG C ON SI.CHGCODE = C.SALCHGCODE
								 WHERE SI.ISTAX = 'N' AND SI.CHGCODE = 1024) DIS
								   ON (M.CODE = DIS.SALINVDET_CODE)
                   WHERE (invcode in (
    select
        unnest(regexp_matches('@DocumentId@',
        '[^|~|]+',
