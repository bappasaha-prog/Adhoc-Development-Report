/* Formatted on 2025/04/08 13:59:52 (QP5 v5.294) */
/*><><>< || Custom Development || Object : CQ_PL_SITE || Ticket Id : 400806 || Developer : Dipankar || ><><><*/
SELECT *
  FROM (  SELECT NULL                     AS REQUESTID,
                 ...
                 END
                    AS LEVEL4,
                 CASE
                    WHEN '@LevelCustom@' IN ('Level5', 'With Ledger')
                    THEN
                       GRP.LEVEL5
                 END
                    AS LEVEL5,
            ...
            FROM (SELECT T.GLCODE,
                         ...
                         CASE
                            WHEN   COALESCE (T.DAMOUNT, 0)
                                 - COALESCE (T.CAMOUNT, 0) >= 0
                            THEN
                                 COALESCE (T.DAMOUNT, 0)
                               - COALESCE (T.CAMOUNT, 0)
                            ELSE
                               0
                         END
                            DEBIT,
                    ...
                    FROM FINPOST P
                         ...
                         LEFT JOIN FINSL S ON T.SLCODE = S.SLCODE
                              ...
                              FROM (SELECT O.GLCODE,
                                     ...
                                     WHERE     G.TYPE IN ('I', 'E')
                                           ...
                                           AND O.ENTDT <
                                                  TO_DATE ('@DTFR@',
                                                           'yyyy-mm-dd')
                                           AND O.ADMOU_CODE = '@ConnOUCode@') P1
                          GROUP BY P1.GLCODE,
                                   P1.GLNAME,
                                   P1.SLCODE,
                                   P1.SLNAME,
