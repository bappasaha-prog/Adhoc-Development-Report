...
WITH DTA
     AS (SELECT TRUNC (MONTH_START_DATE) MTDSTART, TRUNC (SYSDATE) TODAY
  ...
  M.ADMSITE_CODE,
         ...
         SUM (
            CASE
               WHEN TRUNC (M.BILLDATE) = (SELECT TODAY FROM DTA) THEN D.QTY
               ELSE 0
            END)
            BILL_QUANTITY,
         SUM (
   ...
   WHERE TRUNC (M.BILLDATE) BETWEEN (SELECT MTDSTART
                                       FROM DTA)
                                AND (SELECT TODAY
                                       FROM DTA)
