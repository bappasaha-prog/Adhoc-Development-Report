     ...
     AS (  SELECT K.ICODE,
            ...
            WHERE     S.ISPOS = 'Y'
                  ...
                  AND K.ENTTYPE IN ('ADJ',
                                    ...
                                    'GRT',
                                    'STO')
                  AND K.ENTDT::date BETWEEN TO_DATE ('@DTFR@', 'YYYY-MM-DD')
                                          AND TO_DATE ('@DTTO@', 'YYYY-MM-DD')
         GROUP BY K.ICODE, K.ADMSITE_CODE, K.ENTDT::date
         UNION ALL
           SELECT D.ICODE,
                  M.ADMSITE_CODE,
                  M.BILLDATE::date ENTRY_DATE,
                  0                OTHER,
                  0                TOTAL_PURCHASE,
                  0                PURCHASE_RETURN,
                  COALESCE (
            ...
            WHERE     S.ISPOS = 'Y'
                  ...
                  AND M.BILLDATE::date BETWEEN TO_DATE ('@DTFR@',
                                             ...
                                             AND TO_DATE ('@DTTO@',
                                                          'YYYY-MM-DD')
         GROUP BY D.ICODE, M.ADMSITE_CODE, M.BILLDATE::date
         UNION ALL
           SELECT D.ICODE,
                  M.ADMSITE_CODE,
                  M.CSDATE ENTRY_DATE,
            ...
            WHERE     S.ISPOS = 'Y'
                  AND S.ISSECONDARY = 'N'
                  AND CHANNELTYPE = 'ETL'
                  AND M.CSDATE::date BETWEEN TO_DATE ('@DTFR@', 'YYYY-MM-DD')
                                           AND TO_DATE ('@DTTO@', 'YYYY-MM-DD')
