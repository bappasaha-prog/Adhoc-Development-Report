...
*/
  SELECT GINVIEW.FNC_UK () UK,
    ...
    FROM (  SELECT D.ICODE,
             ...
             WHERE     SALETYPE = 'O'
                   AND (   (TRUNC (M.INVDT) BETWEEN TO_DATE ('@Period1DateFrom@',
                                                             'yyyy-mm-dd')
                                                AND TO_DATE ('@Period1DateTo@',
                                                             'yyyy-mm-dd'))
                        OR (TRUNC (M.INVDT) BETWEEN TO_DATE ('@Period2DateFrom@',
                                                             'yyyy-mm-dd')
                                                AND TO_DATE ('@Period2DateTo@',
                                                             'yyyy-mm-dd')))
          GROUP BY D.ICODE, M.ADMSITE_CODE_OWNER, TRUNC (M.INVDT)
          UNION ALL
            SELECT D.ICODE,
                   M.ADMSITE_CODE                     SITECODE,
                   TRUNC (M.CSDATE)                   INVOICE_DATE,
              ...
              FROM SALCSMAIN M
                   ...
                   INNER JOIN GINVIEW.LV_ITEM I ON D.ICODE = I.CODE
             WHERE (   (TRUNC (M.CSDATE) BETWEEN TO_DATE ('@Period1DateFrom@',
                                                          'yyyy-mm-dd')
                                             AND TO_DATE ('@Period1DateTo@',
                                                          'yyyy-mm-dd'))
                    OR (TRUNC (M.CSDATE) BETWEEN TO_DATE ('@Period2DateFrom@',
                                                          'yyyy-mm-dd')
                                             AND TO_DATE ('@Period2DateTo@',
                                                          'yyyy-mm-dd')))
          GROUP BY D.ICODE, M.ADMSITE_CODE, TRUNC (M.CSDATE)
          UNION ALL
            SELECT D.ICODE,
                   M.ADMSITE_CODE                     SITECODE,
                   TRUNC (M.SSDATE)                   INVOICE_DATE,
                   SUM (COALESCE (D.QTY, 0))          SALE_QTY,
                   SUM (COALESCE (D.QTY, 0) * D.COSTRATE) COGS,
                   SUM (COALESCE (D.QTY, 0) * I.MRP)  MRP_VALUE,
                   SUM (COALESCE (D.NETAMT, 0))       NETAMT,
              ...
              FROM SALSSMAIN M
                   ...
                   INNER JOIN GINVIEW.LV_ITEM I ON D.ICODE = I.CODE
             WHERE (   (TRUNC (M.SSDATE) BETWEEN TO_DATE ('@Period1DateFrom@',
                                                          'yyyy-mm-dd')
                                             AND TO_DATE ('@Period1DateTo@',
                                                          'yyyy-mm-dd'))
                    OR (TRUNC (M.SSDATE) BETWEEN TO_DATE ('@Period2DateFrom@',
                                                          'yyyy-mm-dd')
                                             AND TO_DATE ('@Period2DateTo@',
                                                          'yyyy-mm-dd')))
          GROUP BY D.ICODE, M.ADMSITE_CODE, TRUNC (M.SSDATE)) F
